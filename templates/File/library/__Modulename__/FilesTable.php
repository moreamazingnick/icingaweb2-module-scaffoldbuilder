        ...
        return [
            ...
            'action' => [
                ...
                'renderer' => function ($data) {
                    ...
                    $a = Html::tag("a",['target'=>'_next', 'style'=>'padding-right:1em; display:inline;','href'=>Url::fromPath('__modulename__/file/view',['name'=>$data->name])]);
                    $a->add($icon);
                    $div->add($a);

                    $icon=  new Icon('download', ['title' => mt('__modulename__', 'Download')]);
                    $a = Html::tag("a",['target'=>'_blank', 'style'=>'padding-right:1em; display:inline;','href'=>Url::fromPath('__modulename__/file/download',['name'=>$data->name])]);
                    $a->add($icon);
                    $div->add($a);

                    $icon=  new Icon('trash', ['title' => mt('__modulename__', 'Delete')]);
                    $a = Html::tag("a",['target'=>'_self', 'style'=>'padding-right:1em; display:inline;', 'href'=>Url::fromPath('__modulename__/file/delete',['name'=>$data->name])]);
                    $a->add($icon);
                    $div->add($a);
                    return $div;
                }
